...
void *rdma_manager_listen_thread(void *arg)
{
    ...
    while (ctxm->stop_threads == FALSE)
    {
        ...
        for (int i = 0; i < ctxm->ctx_count; i++)
        {
            ...
            // check the cq
            if (ctx->cq == NULL)
                return manager_ret_null(ctx, "CQ is NULL - rdma_manager_liste_thread");
            ...
            if (ibv_post_recv(ctx->conn->qp, &recv_wr, &bad_wr) != 0 || bad_wr)
                return manager_ret_null(ctx, "Failed to post recv - manager_recv");
...
void *rdma_manager_server_thread(void *arg)
{
    ...
    while (ctxm->stop_threads == FALSE)
    {
        ...
        if (ctx_id < 0)
            return manager_ret_null(NULL, "Failed to get free context - rdma_manager_connection_thread");

        // setup the server
        ctx = &ctxm->ctxs[ctx_id];
        if (rdma_server_setup(ctx, ctxm->rdma_port) != 0)
            return manager_ret_null(NULL, "Failed to setup server - rdma_manager_connection_thread");

        if (rdma_server_wait_client_connection(ctx) != 0)
            return manager_ret_null(NULL, "Failed to wait for client connection - rdma_manager_connection_thread");
        ...
        if (ctxm->notification_thread == 0)
        {
            if (rdma_manager_run_listen_th(ctxm) != 0)
                return manager_ret_null(NULL, "Failed to start notification thread - rdma_manager_connection_thread");
