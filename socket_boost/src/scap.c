...
int setup_bpf(bpf_context_t *ctx)
{
    ...
    if (!ctx->obj)
        return ret_err("Failed to open BPF object");

    // load the BPF object file into the kernel
    int err = bpf_object__load(ctx->obj);
    if (err != 0)
        return ret_err("Failed to load BPF object");
    ...
    if (ctx->prog_fd_sockops < 0)
        return ret_err("Failed to getr prog_sock_ops fd");
...
int run_bpf(bpf_context_t *ctx)
{
    ...
    if (!ctx->tcp_destroy_link)
        return ret_err("Failed to attach tcp_destroy_sock_prog to tracepoint");
...
int set_target_ports(bpf_context_t *ctx, __u16 target_ports[], int n, __u16 server_port)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (err != 0)
            return ret_err("Failed to update target_ports map");
    }
    ...
    if (err != 0)
        return ret_err("Failed to update server_port map");
...
int push_sock_to_map(bpf_context_t *ctx, struct sock_id sk_ids[], int n, struct client_sk_t client_sks[])
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        err = bpf_map_update_elem(ctx->free_sk_fd, NULL, &sk_ids[i], BPF_ANY);
        if (err != 0)
            return ret_err("Failed to update free_sockets map");

        // add the socket to the intercepted_sockets map
        int err = bpf_map_update_elem(ctx->intercepted_sk_fd, &sk_ids[i], &client_sks[i].fd, BPF_ANY);
        if (err != 0)
            return ret_err("Failed to add socket to intercepted_sockets map");
    }
    return 0;
}